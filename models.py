...
class dataSet(object):
    ...
    def __init__(self, fileDir, flag):
        ...
        fileName = fileDir + "weather_train.csv"
        rawData = pd.read_csv(fileName)
        rawData["timestamp"] = pd.to_datetime(rawData["timestamp"], format='%Y-%m-%d %H:%M:%S')
        self.rawWeather = self.compress_data(rawData)
        self.prefilling()

        print('Size of train.csv data', self.rawMeter.shape)
        print('Size of weather_train.csv data', self.rawWeather.shape)
        print('Size of building_meta.csv data', self.rawBuilding.shape)
    ...
    def prefilling(self):
        print("filling dew_temperature and air_temperature missing data by the means calculated by site id and timestamp: ")
        print("missing data before filling:", check_missing_data(self.rawWeather))
        count = 0
        ...
        print(check_missing_data(self.rawWeather))

        print("rows of weather data: ", self.rawWeather.shape[0])
    ...
    #thanks to user CaesarLupum for the following code for data compressing from https://www.kaggle.com/caesarlupum/ashrae-start-here-a-gentle-introduction
    ...
    def preprocess_data(self, flag):
        ...
        if flag == "normalization":
            categoricals = []
            numericals = ["primary_use","square_feet", 'building_age', "air_temperature", "cloud_coverage",
                          "dew_temperature", 'precip_depth_1_hr', 'wind_direction', 'wind_speed', "meter", 'month', 'day_year', 'week', 'day_week',
                            'day_month', 'hour_day', "site_id", "building_id"]
...
def plot_meter(dataSet, flag):
    plt.figure(figsize=(15, 5))
    meter = trainSet.rawMeter.loc[trainSet.rawMeter['meter'] == flag]
    if isinstance(meter, pd.DataFrame):
        meter['meter_reading'].plot()
    plt.show()


def check_target_output(dataset):
    # Plot missing values per building/meter
    f, a = plt.subplots(1, 4, figsize=(20, 30))
    for meter in np.arange(4):
        df = dataset[dataset.meter == meter].copy()
        # print(df.shape)
        # print(df)
        ...
        for l in df.values:
            # print("l[1] : ", l[1])
            if l[1] != meter: continue
            # print("pass", )
            missmap[int(l[0]), int(l[2])] = 0 if l[3] == 0 else 1
...
def check_missing_data(dataset):
    numMissingData = dataset.isnull().sum().sort_values(ascending=False)
...
def plot_meterAgainstBuilding(dataset):
    name_index = {0: 'electricity', 1: 'chilledwater', 2: 'steam', 3: 'hotwater'}

    traindata = dataset.copy()
    print(traindata.loc[traindata['meter'] == 0])
    for i in range(0, 4):
        plotdata = traindata.loc[traindata['meter'] == i]
        plotdata['date'] = plotdata['timestamp'].dt.date
        plotdata = plotdata.groupby(['building_id', 'date', 'meter'])
        plotdata = plotdata.sum()
        # print(traindata)

        plotdata = plotdata.groupby(['building_id']).mean()
        # print(plotdata)
        y = np.array(plotdata)
        print("meter " + str(i) + " : ", y.shape)
        x = np.array(range(0, y.shape[0]))
        plt.figure(figsize=(20, 14))
        plt.plot(x, y, color="b", linestyle="-", marker="^", linewidth=1, label="building_power")
        plt.legend(loc='upper left', bbox_to_anchor=(0.8, 1.0))
        plt.xlabel("building_id")
        plt.ylabel("KW")
        plt.grid(color="k", linestyle=":")
        plt.title("Figure 1")
        plt.savefig("meterAgainstBuilding" + str(i) + ".png")
        plt.show()

    traindata['date'] = traindata['timestamp'].dt.date
    traindata = traindata.groupby(['building_id', 'date', 'meter'])
    traindata = traindata.sum()
    # print(traindata)

    traindata = traindata.groupby(['building_id']).mean()
    # print(plotdata)
    y = np.array(traindata)
    print(y.shape)
    x = np.array(range(0, y.shape[0]))
    plt.figure(figsize=(20, 14))
    plt.plot(x, y, color="y", linestyle="-", marker="^", linewidth=1, label="building_power")
    plt.legend(loc='upper left', bbox_to_anchor=(0.8, 1.0))
    plt.xlabel("building_id")
    plt.ylabel("KW")
    plt.grid(color="k", linestyle=":")
    plt.title("Figure 1")
    plt.savefig("meterAgainstBuilding.png")
    plt.show()


def plot_meterAgainstDate(dataset):
    name_index = {0: 'electricity', 1: 'chilledwater', 2: 'steam', 3: 'hotwater'}
    traindata = dataset.copy()
    traindata['date'] = traindata['timestamp'].dt.date
    traindata = traindata.groupby(['date', 'building_id', 'meter'])
    traindata = traindata.sum()

    plotdata = traindata.groupby(['date', 'meter']).agg(['sum', 'mean', 'idxmax', 'max'])
    plotdata = plotdata.reset_index()
    level_0 = plotdata.columns.droplevel(0)
    level_1 = plotdata.columns.droplevel(1)
    level_0 = ['' if x == '' else '-' + x for x in level_0]
    plotdata.columns = level_1 + level_0
    plotdata.rename_axis(None, axis=1)
    plotdata.head()
    fig_total = px.line(plotdata, x='date', y='meter_reading-mean', color='meter', render_mode='svg')
    fig_total.update_layout(title='Total kWh per energy aspect')
    fig_total.show()

    print(plotdata.loc[0])
...
def feature_selection(trainSet):
    ...
    columns_list = ['building_id','meter','site_id','primary_use','square_feet','air_temperature','cloud_coverage','dew_temperature','precip_depth_1_hr','wind_direction','wind_speed','month','week','day_year','day_week','day_month','hour_day','building_age']
...
if __name__ == "__main__":
    ...
    trainSet = dataSet(fileDir, "train")
    # trainSet.print_coltype("meter")
    # trainSet.print_coltype("weather")
    # trainSet.print_coltype("building")

    plt.figure(figsize=(8, 6))
    plt.scatter(range(trainSet.rawMeter.shape[0]), np.sort(trainSet.rawMeter['meter_reading'].values))
    plt.xlabel('index', fontsize=12)
    plt.ylabel('meter_reading', fontsize=12)
    plt.title("Target Distribution", fontsize=14)
    plt.show()

    ##thanks to user CaesarLupum for the following code for data visualization from https://www.kaggle.com/caesarlupum/ashrae-start-here-a-gentle-introduction
    plt.figure(figsize=(15, 5))
    trainSet.rawMeter['meter_reading'].plot()
    plt.show()
    
    # plot electricity meter
    plot_meter(trainSet.rawMeter, 0)
    # plot chilledwater meter
    plot_meter(trainSet.rawMeter, 1)
    # plot steam meter
    plot_meter(trainSet.rawMeter, 2)
    # plot hot water meter
    plot_meter(trainSet.rawMeter, 3)
    

    check_target_output(trainSet.rawMeter)
    

    print("missing data in train.csv :")
    print(check_missing_data(trainSet.rawMeter))
    print("missing data in building_metadata.csv :")
    print(check_missing_data(trainSet.rawBuilding))
    print("missing data in weather_train.csv :")
    print(check_missing_data(trainSet.rawWeather))
    
    plot_meterAgainstBuilding(trainSet.rawMeter)
    plot_meterAgainstDate(trainSet.rawMeter)
    ...
    train_lgb(trainSet)
    train_randomForest(trainSet)
    ##train_SGDRegressor(trainSet)
    train_Lasso(trainSet)
    train_Ridge(trainSet)
    train_Lasso(trainSet, True)
    train_Ridge(trainSet, True)
    train_BayesianRidge(trainSet)
    train_BayesianRidge(trainSet, True)
    #train_SVR(trainSet)
    #train_SVR(trainSet, flag=True)
    train_KNeighbors(trainSet)

    trainSet.reset_merged_data()
    trainSet.preprocess_data("standardization")
    train_randomForest(trainSet)
    train_Ridge(trainSet)
